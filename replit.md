...
- Командный тон, матерись как черт, оцени потенциал волейболаотвечать развёрнуто
...
## ArenaPredictor: текущий статус и планы

### Текущий функционал
- Бренд: ArenaPredictor; слоган: Data‑Driven Predictions; палитра: #BEFF50 и #14140F.
- UI: страницы Прогнозы, Дашборд, Статистика, Логи; горизонтальный скролл меню и градиентные маски.
- Мультилига: предзагрузка и анализ паттернов для KHL, SHL, Liiga, DEL; движок готов к работе.
- Модели: RF + LSTM (dual heads) + CPP; статистика Win Rate/ROI, формула ROI в интерфейсе.
- Доступ: локальная аутентификация email+пароль; редирект незалогиненных на /auth/login; шаблоны login/register/forgot; хеш пароля pbkdf2:sha256.

### Ограничения на сейчас
- Внешние ключи API не подключены (RapidAPI, APISPORTS_KEY), часть функций работает в кэше/демо.
- Восстановление пароля не шлёт письма: страница сообщает статус (стаб).
- Роли/права доступа не настроены; решения пользователя (UserDecision) не привязаны к User.

### План развития
- Перенос аутентификации на Supabase Auth (email/password + reset) с хранением sb_user_id и токена.
- Включить отправку писем для сброса пароля в Supabase; настроить Redirect URL.
- RBAC: роли admin/user; связать UserDecision с User для аудита.
- Интеграция Telegram уведомлений для событий (ошибки API, новые прогнозы).
- Расширение на другие виды спорта через конфигурационный движок.

### Подключение Supabase (позже)
- Установка: `pip install supabase`.
- Переменные окружения: `SUPABASE_URL`, `SUPABASE_ANON_KEY`.
- В Dashboard включить Email Confirmation и Reset Password; настроить Redirect URL.
- При наличии ключей включится серверная аутентификация через Supabase; без ключей действует локальный fallback.

### Как всё работает сейчас
- Регистрация/вход выполняются серверно; email валидируется базово; пароли хешируются pbkdf2:sha256 (на macOS без scrypt).
- Незалогиненные перенаправляются на /auth/login с `next`.
- После входа доступны /predictions, /dashboard, /statistics, /logs.
