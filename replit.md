...
## Результаты (обновлено 2026-01-13)
...
- **Точность:** 70.35% (кросс-валидация: 70.85% ± 3.28%)
- **Исправлено:** Target теперь учитывает паттерны ОБЕИХ команд (home + away)

### Сильные сигналы прерывания (УЛУЧШЕНО!)
| Score | Матчей | Точность прерываний |
|-------|--------|---------------------|
| ≥5 | 1 | **100%** |
| ≥4 | 93 | **72.0%** ⬆️ (было 50.6%) |
| ≥3 | 401 | **56.6%** ⬆️ (было 41.2%) |

**Score** = сумма факторов: синергия ≥2, чередование, перерост серии

### Комбо-сигнал (лучший результат!)
- Score ≥4 + чередование + перерост: **72.9%** прерываний (85 матчей)

### Высокоуверенные прогнозы (продолжения)
| Тип прогноза | Порог | Сигналов | Точность |
|-------------|-------|---------|---------|
| **Продолжение** | ≥70% | 962 | **99.9%** |

**Вывод:** Score ≥4 теперь даёт 72% точность для прерываний!
