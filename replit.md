...
## Результаты Random Forest (обновлено 2026-01-13)
...
- **Точность:** 54.48% (кросс-валидация: 53.22% ± 1.66%)
- **Базовая частота побед хозяев:** 54.3%

### Выводы
- Модель теперь предсказывает **конкретный исход** (кто победит)
- Паттерны дают небольшое преимущество над базовой частотой (~0.2%)
- Для улучшения нужны дополнительные признаки (форма, травмы, статистика)
...
### Паттерны (справочно)
- Общее чередование: 58.1% прерываний
- Домашнее чередование: 55.6% прерываний
- Синергия 2+ критических: 51.5% прерываний
